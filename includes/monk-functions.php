...
function monk_get_translations( $element_id = null ) {
	...
	if ( is_archive() && ( is_category() || is_tag() || is_tax() ) ) {
		...
		if ( $monk_translations ) {
			...
			$current_language_slug                       = $monk_languages[ get_option( 'monk_default_language', false ) ]['slug'];
...
/**
 * Filter to handle and print hreflang tags in the HEAD section
 *
 * @since 0.7.0
 *
 ...
 */
function monk_filter_localization_tags( $custom_id = null, $custom_lang_data = array() ) {
	if ( is_admin() ) {
		return;
	}
...
add_filter( 'monk_localization_tags', 'monk_filter_localization_tags', 10, 2 );
